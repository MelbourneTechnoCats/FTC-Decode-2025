...
//        registerGroupedCommands(commands); // NOTE: very janky fix to satisfy Java since we cannot have this in the FTCLib package
    ...
    public void end(boolean interrupted) {
        if (
                interrupted && !m_commands.isEmpty()
                && m_currentCommandIndex >= 0 && m_currentCommandIndex < m_commands.size() // FIX: implement command index check from WPILib
        ) {
